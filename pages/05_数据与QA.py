...
if not show_empty(coverage, "mapping_coverage.csv 为空"):
    coverage_zh = coverage.rename(columns={
        ...
        "mapped_asin_count": "映射成功ASIN数",
        "mapping_rate": "映射率",
    }).copy()
    if "映射率" in coverage_zh.columns:
        coverage_zh["映射率"] = coverage_zh["映射率"] * 100
...
if not show_empty(unmapped, "unmapped_asin.csv 为空"):
    unmapped_zh = unmapped.rename(columns={
        "segment_raw": "原型号段",
        "asin": "ASIN",
        "brand": "品牌",
        "model": "型号",
    ...
    show_cols = [c for c in show_cols if c in unmapped_zh.columns]
    st.dataframe(unmapped_zh[show_cols], use_container_width=True, hide_index=True, column_config=make_brand_table_config(show_cols))
